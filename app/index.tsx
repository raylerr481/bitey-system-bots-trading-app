...
const COPY = { es: { title: "Bitey SBT", subtitle: "Trading intelligence y mercado en tiempo real", market: "Mercado", timeframe: "Temporalidad", connected: "CONECTADO", disconnected: "SIN CONEXIÓN", price: "Precio", bid: "Bid", ask: "Ask", spread: "Spread", gateway: "Bitey SBT Market Gateway", analyze: "ANALIZAR MERCADO", event: "Evento", result: "Resultado", note: "SBT es la plataforma de trading. MT5, Alpaca, TradingView y otros proveedores son conectores de mercado/ejecución; las credenciales no pasan por la app." }, pt: { title: "Bitey SBT", subtitle: "Inteligência de trading e mercado em tempo real", market: "Mercado", timeframe: "Período", connected: "CONECTADO", disconnected: "SEM CONEXÃO", price: "Preço", bid: "Bid", ask: "Ask", spread: "Spread", gateway: "Bitey SBT Market Gateway", analyze: "ANALISAR MERCADO", event: "Evento", result: "Resultado", note: "SBT é a plataforma de trading. MT5, Alpaca, TradingView e outros provedores são conectores; as credenciais não passam pelo app." }, en: { title: "Bitey SBT", subtitle: "Trading intelligence and real-time market", market: "Market", timeframe: "Timeframe", connected: "CONNECTED", disconnected: "DISCONNECTED", price: "Price", bid: "Bid", ask: "Ask", spread: "Spread", gateway: "Bitey SBT Market Gateway", analyze: "ANALYZE MARKET", event: "Event", result: "Result", note: "SBT is the trading platform. MT5, Alpaca, TradingView and other providers are market/execution connectors; credentials never pass through the app." } } as const;
...
export default function Home() {
  ...
  useEffect(() => {
    setTick(null); setPrices([]); setConnected(false);
    const socket = new WebSocket(wsUrl(apiUrl, symbol));
    socketRef.current = socket;
    socket.onopen = () => setConnected(true);
    socket.onmessage = (message) => { try { const data = JSON.parse(message.data) as Tick; setTick(data); const value = Number(data.last ?? data.bid ?? data.ask); if (Number.isFinite(value)) setPrices((current) => [...current.slice(-59), value]); setConnected(!data.error); } catch { setConnected(false); } };
  ...
  const chartPoints = useMemo(() => { if (!prices.length) return []; const min = Math.min(...prices); const max = Math.max(...prices); const range = Math.max(max - min, Math.abs(max) * 0.000001, 0.000001); return prices.map((value) => 8 + ((value - min) / range) * 92); }, [prices]);
